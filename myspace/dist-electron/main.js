...
app.on("ready", () => {
  const { protocol: protocol2 } = electron;
  protocol2.handle("media", (request) => {
    const filePath = request.url.slice("media://".length);
    const decodedPath = decodeURIComponent(filePath);
    try {
      let cleanPath = decodedPath;
      if (process.platform === "win32" && cleanPath.startsWith("/") && !cleanPath.startsWith("//")) {
        cleanPath = cleanPath.slice(1);
      }
      return electron.net.fetch(`file:///${cleanPath}`);
    } catch (e) {
  ...
  ipcMain.handle("get-files", async (event, dirPath) => {
    console.log("IPC: get-files called for path:", dirPath);
    try {
      const files = await import_fs.default.promises.readdir(dirPath, { withFileTypes: true });
      console.log(`Found ${files.length} files in ${dirPath}`);
      const mappedFiles = files.map((file) => ({
        name: file.name,
        isDirectory: file.isDirectory(),
        path: import_path.default.join(dirPath, file.name)
      }));
      console.log("Mapped files:", mappedFiles.slice(0, 5));
      return mappedFiles;
    } catch (error) {
