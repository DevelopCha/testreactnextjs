...
var { app, BrowserWindow, ipcMain, dialog, protocol, Menu, shell, net } = electron;
...
var createWindow = () => {
  mainWindow = new BrowserWindow({
    ...
    // dark gray-900 to prevent white flash
    webPreferences: {
      preload: import_path.default.join(__dirname, "preload.cjs"),
      nodeIntegration: false,
      contextIsolation: true,
      webSecurity: false
      // Temporarily disable webSecurity to rule out any other blocking issues
    }
  });
...
app.on("ready", () => {
  protocol.handle("media", async (request) => {
    try {
      ...
      const absolutePath = import_path.default.resolve(p);
      ...
      const nodeStream = import_fs.default.createReadStream(absolutePath);
      ...
      submenu: [
        {
          ...
          click: async () => {
            ...
            if (!result.canceled && result.filePaths.length > 0) {
              mainWindow == null ? void 0 : mainWindow.webContents.send("folder-selected", result.filePaths[0]);
            }
          }
        },
  ...
  ipcMain.handle("get-files", async (event, dirPath) => {
    try {
      async function getEntriesRecursively(currentPath) {
        const entries = await import_fs.default.promises.readdir(currentPath, { withFileTypes: true });
        const result = [];
  ...
  ipcMain.handle("rename-file", async (event, oldPath, newName) => {
    ...
    try {
      ...
      try {
        await import_fs.default.promises.access(newPath);
        return { success: false, error: "Target file already exists. Use Batch Rename for swapping names." };
      } catch (e) {
      }
      await import_fs.default.promises.rename(oldPath, newPath);
  ...
  ipcMain.handle("batch-rename-files", async (event, rootPath, operations) => {
    console.log(`IPC: batch-rename-files called with ${operations.length} operations in ${rootPath}`);
    const tempDir = import_path.default.join(rootPath, ".ag_temp_batch");
    const results = { success: true };
    try {
      if (!import_fs.default.existsSync(tempDir)) {
        await import_fs.default.promises.mkdir(tempDir);
      }
      const tempMap = {};
      for (const op of operations) {
        const fileName = import_path.default.basename(op.oldPath);
        const tempPath = import_path.default.join(tempDir, fileName);
        try {
          await import_fs.default.promises.rename(op.oldPath, tempPath);
      ...
      for (const op of operations) {
        ...
        const finalPath = import_path.default.join(rootPath, op.newName);
        try {
          await import_fs.default.promises.rename(tempPath, finalPath);
        } catch (e) {
          console.error(`Failed to move ${import_path.default.basename(tempPath)} to ${op.newName}:`, e);
          throw new Error(`Failed to restore ${op.newName}: ${e.message}`);
        }
      }
      await import_fs.default.promises.rmdir(tempDir);
      return { success: true };
    } catch (error) {
      console.error("Batch rename failed:", error);
      try {
        if (import_fs.default.existsSync(tempDir) && (await import_fs.default.promises.readdir(tempDir)).length === 0) {
          await import_fs.default.promises.rmdir(tempDir);
        }
      } catch (cleanupErr) {
  ...
  ipcMain.handle("open-zip", async (event, zipPath) => {
    console.log("IPC: open-zip called for:", zipPath);
    try {
      const zip = new import_adm_zip.default(zipPath);
      const zipEntries = zip.getEntries();
      const zipName = import_path.default.basename(zipPath);
      const tempDirName = `.ag_zip_${zipName}_${Date.now()}`;
      const tempDirPath = import_path.default.join(import_path.default.dirname(zipPath), tempDirName);
  ...
  ipcMain.handle("get-zip-cover", async (event, zipPath) => {
    try {
      const zipName = import_path.default.basename(zipPath);
      const coverCacheDir = import_path.default.join(import_path.default.dirname(zipPath), ".ag_covers");
      if (!import_fs.default.existsSync(coverCacheDir)) {
        import_fs.default.mkdirSync(coverCacheDir, { recursive: true });
      }
      const zip = new import_adm_zip.default(zipPath);
      ...
      const coverPath = import_path.default.join(coverCacheDir, `${zipName}_${safeEntryName}`);
      if (import_fs.default.existsSync(coverPath)) {
        return coverPath;
      }
      zip.extractEntryTo(imageEntry, coverCacheDir, false, true);
      const buffer = zip.readFile(imageEntry);
      if (buffer) {
        await import_fs.default.promises.writeFile(coverPath, buffer);
        return coverPath;
      }
      return null;
    } catch (e) {
